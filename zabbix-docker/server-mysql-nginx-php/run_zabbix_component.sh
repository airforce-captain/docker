...
prepare_zbx_web_config() {
    ...
    if [ -n "$PHP_CONFIG_FILE" ]; then
        ...
        update_config_var "$PHP_CONFIG_FILE" "date.timezone" "${TZ:-"Asia/Shanghai"}"
    else
...
#chenge php timezone
sed -i "s/\; date.timezone=Europe\/Riga/date.timezone=Asia\/Shanghai/" /etc/php5/conf.d/99-zabbix.ini

#change zabbix configuration
sed -i \
    -e "s/{DB_SERVER_HOST}/${DB_SERVER_HOST}/g" \
    -e "s/{DB_SERVER_PORT}/${DB_SERVER_PORT}/g" \
    -e "s/{DB_SERVER_DBNAME}/${DB_SERVER_DBNAME}/g" \
